...
You will need to enable CORS requests in the browser for this example to work. On a Chrome Browser, you can use this [Chrome plugin](https://chrome.google.com/webstore/detail/forcecors/oajaiobpeddomajelicdlnkeegnepbin?hl=en) to enable CORS requests. After you install the plugin go to ForceCORS Settings. Add the following :

* URL - **https://api.brightcove.com/services/library***
* Header : Access-Control-Allow-Origin" 
* Header value : *

Then save the settings and load the sample.

NOTE: 'timeupdate' and durationchange' event for Ads may not be triggered on the browser
